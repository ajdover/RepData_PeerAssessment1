...
```{r}
DataFile.new.mean.interval <- DataFile.new[,list(meanSteps = mean(steps, na.rm=TRUE)), list(dayType, interval)]

ggplot(DataFrame.new.mean.interval, aes(x=interval, y-meanSteps, color=dayType)) + geom_line() + facet_wrap(~ dayTime, nrow=2) + theme(legend.position="none")
